...
Genome = Nbit*Nvar
generation_max = 100    # Define here the maximum number of iterations
...
gbest_chrom = np.empty([genomeLength], dtype=int)
gbest_fitness = 0
pbest_chrom = np.empty([popSize, genomeLength], dtype=int)
...
best_pchrom = np.empty([generation_max], dtype=int)
    ...
    for i in range(1, popSize):
        for j in range(1, genomeLength):
            ...
            if fitness[i] < gbest_fitness:
                ...
                if chromosome[i, j] == 0 and gbest_chrom[j] == 1:
                    # Define the rotation angle: delta_theta (e.g. 0.0785398163)
                    if (g_alpha * g_beta > 0):
                        delta_theta = theta
                    elif (g_alpha * g_beta < 0):
                        delta_theta = -theta
                    elif (g_beta == 0):
                        delta_theta = -theta
                if chromosome[i, j] == 1 and gbest_chrom[j] == 0:
                    # Define the rotation angle: delta_theta (e.g. -0.0785398163)
                    if (g_alpha * g_beta > 0):
                        delta_theta = -theta
                    elif (g_alpha * g_beta < 0):
            ...
            # if 𝑓(𝑥) > 𝑓(pbest): FALSE
            if fitness[i] < pbest_fitness[i]:
                # if chromosome[i,j]==0 and chromosome[best_chrom[generation],j]==0:
                if chromosome[i, j] == 0 and pbest_chrom[i, j] == 1:
                    # Define the rotation angle: delta_theta (e.g. 0.0785398163)
                    if (p_alpha * p_beta > 0):
                        delta_theta = theta
                    elif (p_alpha * p_beta < 0):
                        delta_theta = -theta
                    elif (p_beta == 0):
                        delta_theta = -theta
                if chromosome[i, j] == 1 and pbest_chrom[i, j] == 0:
                    # Define the rotation angle: delta_theta (e.g. -0.0785398163)
                    if (p_alpha * p_beta > 0):
                        delta_theta = -theta
                    elif (p_alpha * p_beta < 0):
