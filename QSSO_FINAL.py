...
Genome = Nbit*Nvar
generation_max = 500    # Define here the maximum number of iterations
...
#########################################################
    ...
    fp = open("QSSO_pbest.dat", "a")
    f.write(str(generation)+" "+str(gbest_fitness)+"\n")
    for i in range(1, popSize):
        fp.write(str(generation)+" "+str(i)+" "+str(pbest_fitness[i])+"\n")
    f.write(" \n")
    fp.write(" \n")
    f.close()
    fp.close()
...
def get_theta_GADD_Linear():
    global generation
    global generation_max
    theta_max = 0.1 * np.pi
    theta_min = 0.0025 * np.pi
    delta_theta = theta_max - (theta_max - theta_min) * \
...
def rotation():
    ...
    for i in range(1, popSize):
        for j in range(1, genomeLength):
            ...
            # delta_theta = get_theta_GADD_Linear()
            rot = get_rot(delta_theta)
...
def plot_Output():
    ...
    for i in range(N):
        plt.plot(pbest_list[i], color=color[i], linewidth=0.5)
